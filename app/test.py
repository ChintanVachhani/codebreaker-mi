...
def test():
    ...
    with g.graph.as_default():
        ...
        with sv.managed_session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            _preds = copy.copy(x)
            while 1:
                istarget, probs, preds = sess.run([g.istarget, g.probs, g.preds], {g.x: _preds})
                ...
                print("pred here")
                print(_preds)
                _preds = np.where(x == 0, _preds, x)  # # Fill in the non-blanks with correct numbers
                print("after filling with correct numbers")
                print(_preds)
